...
def load_label(path):
    path_photo = path  # 所有photo所在的文件夹目录
    #
    files_list = os.listdir(path_photo)  # 得到文件夹下的所有文件名称，存在字符串列表中
    #

    label_list = os.path.join(path, 'test.txt')
    with open(label_list, 'r') as f:
        files_list = f.read().split('\n')

    # files_list = open(label_list, encoding="utf-8").read()
    ...
    for i in files_list:
        ...
        # label_test = os.path.join(path, 'test.txt')
        labels.append((label))
        # np.savetxt('/remote-home/jywang/result/test/test.txt', labels[0], fmt="%d", delimiter=" ")
...
def main():
    ...
    # dataset_type = config['val_loader']['type']

    assert dataset_type in ['VOC', 'COCO', 'CityScapes', 'ADE20K', 'Drionsdb', "Eye", "Rimone", "Drishtigs1"]
    ...
    with torch.no_grad():
        ...
        for img_file in tbar:
            ...
            # print('pre_size', prediction.size)
            # print(prediction)
            results.append(prediction)
            # save_images(image, prediction, args.output, img_file, palette)
            # save_images(image, labels[i], args.output, img_file, palette)
            # i = i + 1

        # print('result', results)
        # print('labels', labels)
        all_acc, acc, iou = metrics.mean_iou(results, labels, 2, -100)
        print('mIoU', iou)

def parse_arguments():
    parser = argparse.ArgumentParser(description='Inference')
    parser.add_argument('-c', '--config', default=r'/remote-home/jywang/pytorch_semantic_tj/saved/ENet/05-11_14-40/config.json', type=str,
                        help='The config used to train the model')
    parser.add_argument('-mo', '--mode', default='multiscale', type=str,
                        help='Mode used for prediction: either [multiscale, sliding]')
    parser.add_argument('-m', '--model', default=r'/remote-home/jywang/pytorch_semantic_tj/saved/ENet/05-11_14-40/best_model.pth', type=str,
                        help='Path to the .pth model checkpoint to be used in the prediction')
    parser.add_argument('-i', '--images', default=r'/remote-home/jywang/share/optic_disc/Drishti-GS1_files/test_image', type=str,
                        help='Path to the images to be segmented')
    parser.add_argument('-l', '--labels', default=r'/remote-home/jywang/share/optic_disc/Drishti-GS1_files/test_label', type=str,
                        help='Path to the labels to be segmented')
    parser.add_argument('-o', '--output', default=r'/remote-home/jywang/result/test', type=str,
                        help='Output Path')
