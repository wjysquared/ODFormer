...
def intersect_and_union(pred_label,
    ...
    tn = np.argwhere((pred_label == 0) & (label == 0))

    for index_tn in tn:
        input_img[index_tn[0], index_tn[1]] = 79

    TN = input_img[input_img == 79]
    area_tn, _ = np.histogram(
        TN, bins=np.arange(num_classes + 1))
    print("TN", area_tn)
